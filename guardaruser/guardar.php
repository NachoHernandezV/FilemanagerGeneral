...
//include 'conexion.php';
		...
		$con=mysqli_connect($server,$username,$passwordz,$db);
		...
		$password=$_POST[pass];
		$departamentosselect=$_POST[departamentos];
		$nivelpermisouser=$_POST[nivelpermiso];
		///
		
		if($_POST[Subir] == "Subir")
		{
			$consulta="INSERT INTO usuarios (nombre,password,departamento,nivelpermiso) VALUES ('$nombreusuario','$password','$departamentosselect[0]','$nivelpermisouser')";
			mysqli_query($con,$consulta);
		}

echo $nombreusuario.$password.$departamentosselect[0].$nivelpermisouser;
...
$Ventas=$_POST[Ventas];
$DirecciondeVentas=$_POST[DirecciondeVentas];
$DirecciondeCalidad=$_POST[DirecciondeCalidad];
$DirecciondeProduccion=$_POST[DirecciondeProduccion];
$GerenciaInvestigacion=$_POST[GerenciaInvestigacion];
$GerenciaCedi=$_POST[GerenciaCedi];
$GerenciaMezclado=$_POST[GerenciaMezclado];
$GerenciaPlaneacion=$_POST[GerenciaPlaneacion];
...
if($DirecciondeVentas=="on" or $departamento=="DirecciondeVentas")
	$DirecciondeVentas="si";
else
	$DirecciondeVentas="no";
	
if($DirecciondeCalidad=="on" or $departamento=="DirecciondeCalidad")
	$DirecciondeCalidad="si";
else
	$DirecciondeCalidad="no";

if($DirecciondeProduccion=="on" or $departamento=="DirecciondeProduccion")
	$DirecciondeProduccion="si";
else
	$DirecciondeProduccion="no";

if($GerenciaInvestigacion=="on" or $departamento=="GerenciaInvestigacion")
	$GerenciaInvestigacion="si";
else
	$GerenciaInvestigacion="no";

if($GerenciaCedi=="on" or $departamento=="GerenciaCedi")
	$GerenciaCedi="si";
else
	$GerenciaCedi="no";

if($GerenciaMezclado=="on" or $departamento=="GerenciaMezclado")
	$GerenciaMezclado="si";
else
	$GerenciaMezclado="no";


if($GerenciaPlaneacion=="on" or $departamento=="GerenciaPlaneacion")
	$GerenciaPlaneacion="si";
else
	$GerenciaPlaneacion="no";
	...
	*/
	...
	if($_POST[Subir]=="Subir")
	{
		...
		*/
		...
		$consulta3="Select * from usuarios where nombre='".$nombreusuario."' and password='".$password."' and departamento='".$departamentosselect[0]."' and nivelpermiso='".$nivelpermisouser."'";
		$query=mysqli_query($con,$consulta3);
		mysqli_data_seek($query,0);
		$idarchivo = mysqli_fetch_row($query);
		
		echo $consulta3."   EXITO";
		
		$permisosuser="insert into permisosusuarios(Almacen,Basculas,Calidad,Cedi,Compras,Contabilidad,Credito,DireccionAdministrativa,DireccionGeneral,Embarques,Envasado,SanidadyTrigos,Gestiondepersonal,Investigacion,LabFuncionales,LabHarinas,LabMicrobiologia,Legal,Mantenimiento,Mezclas,Molino,Panaderia,Planeacion,Recepcion,Ribera,SeguridadIndustrial,Sistemas,Tesoreria,Ventas,DirecciondeVentas,DirecciondeCalidad,DirecciondeProduccion,GerenciaInvestigacion,GerenciaCedi,GerenciaMezclado,GerenciaPlaneacion,idusuario) values('$Almacen','$Basculas','$Calidad','$Cedi','$Compras','$Contabilidad','$Credito','$DireccionAdministrativa','$DireccionGeneral','$Embarques','$Envasado','$SanidadyTrigos','$GestiondePersonal','$Investigacion','$LabFuncionales','$LabHarinas','$LabMicrobiologia','$Legal','$Mantenimiento','$Mezclas','$Molino','$Panaderia','$Planeacion','$Recepcion','$Ribera','$SeguridadIndustrial','$Sistemas','$Tesoreria','$Ventas','$DirecciondeVentas','$DirecciondeCalidad','$DirecciondeProduccion','$GerenciaInvestigacion','$GerenciaCedi','$GerenciaMezclado','$GerenciaPlaneacion','$idarchivo[0]')";
		
		mysqli_query($con,$permisosuser);
